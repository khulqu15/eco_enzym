    ...
    <div class="px-8 py-8">
      ...
      <div class="bg-base-100 relative w-full rounded-2xl mt-6 overflow-hidden">
        <div v-if="user" class="w-full p-8 relative items-center flex gap-x-4">
            ...
            <div>
                <h3 class="m-0 font-bold">{{ user.displayName }}</h3>
                <p class="text-base-content">{{ user.email }}</p>
            </div>
        </div>
        <div class="w-full bg-primary p-2 rounded-2xl"></div>
      </div>
      <div @click="changeTheme()" class="btn bg-base-300 h-24 w-full p-6 rounded-2xl mt-4 flex items-center justify-between">
        ...
        <div>
            <p class="text-sm text-end mt-3 text-base-content">Click to change</p>
        </div>
      </div>
    </div>
